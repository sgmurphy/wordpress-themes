...
class Customizer_Repeater extends WP_Customize_Control {
	...
	public $customizer_repeater_btn_text_control = false;
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->add_field_label = esc_html__( 'Add new field', 'ashe' );
		if ( ! empty( $args['add_field_label'] ) ) {
			$this->add_field_label = $args['add_field_label'];
		}

		$this->boxtitle = esc_html__( 'Customizer Repeater', 'ashe' );
		...
		if ( ! empty( $args['customizer_repeater_btn_text_control'] ) ) {
			$this->customizer_repeater_btn_text_control = $args['customizer_repeater_btn_text_control'];
		}
			...
			foreach($array as $icon){ ?>
                <div class="customizer-repeater-general-control-repeater-container customizer-repeater-draggable">
                    ...
                    <div class="customizer-repeater-box-content-hidden">
						<?php
						$choice = $image_url = $icon_value = $title = $subtitle = $text = $checkbox = $text2 = $btn_text = $link2 = $link = $shortcode = $repeater = $color = $color2 = '';
						if(!empty($icon->id)){
							$id = $icon->id;
						}
						...
						if(!empty($icon->btn_text)){
							$btn_text = $icon->btn_text;
						}
						if(!empty($icon->text2)){
							$text2 = $icon->text2;
						}
						...
						if($this->customizer_repeater_color_control == true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Color','ashe' ), $this->id, 'customizer_repeater_color_control' ),
						...
						if($this->customizer_repeater_color2_control == true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Color','ashe' ), $this->id, 'customizer_repeater_color2_control' ),
						...
						if($this->customizer_repeater_title_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Title','ashe' ), $this->id, 'customizer_repeater_title_control' ),
						...
						if($this->customizer_repeater_subtitle_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Subtitle','ashe' ), $this->id, 'customizer_repeater_subtitle_control' ),
						...
						if($this->customizer_repeater_text_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Description','ashe' ), $this->id, 'customizer_repeater_text_control' ),
								'class' => 'customizer-repeater-text-control',
								'type'  => apply_filters('customizer_repeater_input_types_filter', 'textarea', $this->id, 'customizer_repeater_text_control' ),
							), $text);
						}
						if($this->customizer_repeater_btn_text_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Button Text','ashe' ), $this->id, 'customizer_repeater_btn_text_control' ),
								'class' => 'customizer-repeater-button-text-control',
								'type'  => apply_filters('customizer_repeater_input_types_filter', '', $this->id, 'customizer_repeater_btn_text_control' ),
							), $btn_text);
						}
						if($this->customizer_repeater_link_control){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Slide Link','ashe' ), $this->id, 'customizer_repeater_link_control' ),
								'class' => 'customizer-repeater-link-control',
								'sanitize_callback' => 'esc_url_raw',
								'type'  => apply_filters('customizer_repeater_input_types_filter', '', $this->id, 'customizer_repeater_link_control' ),
							), $link);
						}
						if($this->customizer_repeater_checkbox_control){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Open Link in New Tab','ashe' ), $this->id, 'customizer_repeater_checkbox_control' ),
								'class' => 'customizer-repeater-checkbox-control',
								'type'  => apply_filters('customizer_repeater_input_types_filter', 'checkbox', $this->id, 'customizer_repeater_checkbox_control' ),
							), $checkbox);
						}
						if($this->customizer_repeater_text2_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Text','ashe' ), $this->id, 'customizer_repeater_text2_control' ),
						...
						if($this->customizer_repeater_link2_control){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Link','ashe' ), $this->id, 'customizer_repeater_link2_control' ),
						...
						if($this->customizer_repeater_shortcode_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Shortcode','ashe' ), $this->id, 'customizer_repeater_shortcode_control' ),
                        ...
                        <button type="button" class="social-repeater-general-control-remove-field" <?php if ( $it == 0 ) {
							...
							<?php esc_html_e( 'Delete field', 'ashe' ); ?>
                        </button>
		...
		if( !empty($options['type']) ){
			switch ($options['type']) {
				case 'textarea':?>
                    ...
                    <textarea class="<?php echo esc_attr( $options['class'] ); ?>"><?php echo ( !empty($options['sanitize_callback']) ?  call_user_func_array( $options['sanitize_callback'], array( $value ) ) : esc_attr($value) ); ?></textarea>
					<?php
					break;
				case 'color':
					$style_to_add = '';
				...
				?>
                    <div class="<?php echo esc_attr($options['class']); ?>">
                   		<span class="customize-control-title">
	                   		<input type="checkbox" value="<?php echo ( !empty($options['sanitize_callback']) ?  call_user_func_array( $options['sanitize_callback'], array( $value ) ) : esc_attr($value) ); ?>" class="<?php echo esc_attr($options['class']); ?>" />
	...
	private function image_control($value = '', $show = ''){ ?>
        <div class="customizer-repeater-image-control">
            <span class="customize-control-title">
                <?php esc_html_e('Slide Background','ashe'); ?>
            </span>
            <?php echo wp_get_attachment_image($value); ?>
            <input type="text" class="widefat custom-media-url" value="<?php echo esc_attr( $value ); ?>">
            <input type="button" class="button button-secondary customizer-repeater-custom-media-button" value="<?php esc_attr_e( 'Upload Image','ashe' ); ?>" />
        </div>
		<?php
	}
	...
	private function icon_type_choice($value='customizer_repeater_icon'){ ?>
        <span class="customize-control-title">
            <?php esc_html_e('Image type','ashe');?>
        </span>
        <select class="customizer-repeater-image-choice">
            <option value="customizer_repeater_icon" <?php selected($value,'customizer_repeater_icon');?>><?php esc_html_e('Icon','ashe'); ?></option>
            <option value="customizer_repeater_image" <?php selected($value,'customizer_repeater_image');?>><?php esc_html_e('Image','ashe'); ?></option>
            <option value="customizer_repeater_none" <?php selected($value,'customizer_repeater_none');?>><?php esc_html_e('None','ashe'); ?></option>
        </select>
		<?php
	}
	...
	private function repeater_control($value = ''){
		...
		$show_del        = 0; ?>
        <span class="customize-control-title"><?php esc_html_e( 'Social icons', 'ashe' ); ?></span>
		<?php
            ...
            <div class="customizer-repeater-social-repeater">
                <div class="customizer-repeater-social-repeater-container">
                    ...
                    <button class="social-repeater-remove-social-item" style="display:none">
						<?php esc_html_e( 'Remove Icon', 'ashe' ); ?>
                    </button>
                </div>
                <input type="hidden" id="social-repeater-socials-repeater-colector" class="social-repeater-socials-repeater-colector" value=""/>
            </div>
            <button class="social-repeater-add-social-item button-secondary"><?php esc_html_e( 'Add Icon', 'ashe' ); ?></button>
			<?php
		} else { ?>
            <div class="customizer-repeater-social-repeater">
				<?php
                    ...
                    <div class="customizer-repeater-social-repeater-container">
                        ...
                        <button class="social-repeater-remove-social-item"
                                style="<?php if ( $show_del == 1 ) {
							        echo "display:none";
						        } ?>"><?php esc_html_e( 'Remove Icon', 'ashe' ); ?></button>
                    </div>
					<?php
				} ?>
