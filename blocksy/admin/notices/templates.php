...
add_action('admin_notices', function () {
	...
	blocksy_output_blocks_drop_notice();
});

function blocksy_output_blocks_drop_notice() {
	if (! current_user_can('activate_plugins')) return;
	if (get_option('dismissed-blocksy_blocks_drop_notice', false)) return;

	$blocksy_plugin_notice = get_option(
		'dismissed-blocksy_plugin_notice',
		false
	);

	if (! $blocksy_plugin_notice) {
		return;
	}
	...
	echo '<div class="notice notice-blocksy-blocks-move">';
	echo '<div class="notice-blocksy-blocks-move-root" data-url="' . esc_attr($url) . '" data-plugin-url="' . esc_attr($plugin_url) . '" data-plugin-status="' . esc_attr($status) . '" data-link="' . esc_attr($plugin_link) . '">';

	?>

	<div class="ct-blocksy-blocks-move-inner">
