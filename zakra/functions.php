...
add_action(
	...
	function ( $product_id, $attachment_id ) {
		$checked = get_post_meta( $product_id, '_zakra_product_gallery_image_id_' . $attachment_id, true );
        $checked = $checked ? '1' : '0';
		?>
	<input type="checkbox" value="1" name="<?php echo esc_attr( '_zakra_product_gallery_image_id_' . $attachment_id ); ?>" <?php checked( $checked, '1' ); ?>>
		<?php
	},
	10,
	2
...
add_action(
	...
	function ( $post_id ) {
		$posted = $_POST; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$gallery_image_ids = wc_get_product( $post_id )->get_gallery_image_ids();
		foreach ( $gallery_image_ids as $id ) {
			if ( isset( $posted[ '_zakra_product_gallery_image_id_' . $id ] ) && $posted[ '_zakra_product_gallery_image_id_' . $id ] ) {
				update_post_meta( $post_id, '_zakra_product_gallery_image_id_' . $id, 1 );
			} else {
...
add_action(
	...
	function() {
		...
		foreach ( $gallery_image_ids as $id ) {
			$ischecked = get_post_meta( $product->get_id(), '_zakra_product_gallery_image_id_' . $id, true );
            error_log( print_r( $ischecked, true ) );
