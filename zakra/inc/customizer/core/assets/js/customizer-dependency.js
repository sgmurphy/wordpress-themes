...
(
	function ( $ ) {
		 ...
		 */
		var Zakra_Customizer_FrameWork = {
			...
			checkControlVisibility : function ( control, id ) {
				...
				if ( ! _.isUndefined( control ) ) {
					...
					if ( 'undefined' != typeof ZakraCustomizerControlsToggle[id] ) {
						var check            = false,
						    dependency_param = ZakraCustomizerControlsToggle[id],
						...
						if ( 'undefined' !== typeof conditions ) {
							...
							if ( ! check ) {
								control.container.addClass( 'zakra-hide' );
							} else {
								control.container.removeClass( 'zakra-hide' );
							}
						}
					}
			...
			checkDependency : function ( conditions, values, compare_operator ) {
				...
				if ( _.isString( testValue ) ) {
					...
					if ( ! _.isUndefined( ZakraCustomizerControlsToggle[testValue] ) ) {
						var conditions = ! _.isUndefined( ZakraCustomizerControlsToggle[testValue]['conditions'] ) ? ZakraCustomizerControlsToggle[testValue]['conditions'] : ZakraCustomizerControlsToggle[testValue];
						var operator   = ! _.isUndefined( ZakraCustomizerControlsToggle[testValue]['operator'] ) ? ZakraCustomizerControlsToggle[testValue]['operator'] : 'AND';
						...
						if ( ! _.isUndefined( conditions ) ) {
							...
							if ( ! control.checkDependency( conditions, values, operator ) ) {
								...
								control_obj.container.removeClass( 'zakra-hide' );
							}
						}
					}
					...
					$.each(
						...
						function ( index, val ) {
							...
							if ( 'undefined' !== typeof ZakraCustomizerControlsToggle[cond_key] ) {
								var conditions = ! _.isUndefined( ZakraCustomizerControlsToggle[cond_key]['conditions'] ) ? ZakraCustomizerControlsToggle[cond_key]['conditions'] : ZakraCustomizerControlsToggle[cond_key];
								var operator   = ! _.isUndefined( ZakraCustomizerControlsToggle[cond_key]['operator'] ) ? ZakraCustomizerControlsToggle[cond_key]['operator'] : 'AND';
			...
			hideEmptySections : function () {
				...
				$( 'ul.accordion-section.control-section-zakra_section' ).each(
					function () {
						...
						if ( controls.length > 0 ) {
							controls.each(
								function () {
									if ( ! $( this ).hasClass( 'zakra-hide' ) && $( this ).css( 'display' ) != 'none' ) {
										visibleIt = true;
									}
								}
							);
							...
							if ( ! visibleIt ) {
								$( '.control-section[aria-owns="' + parentId + '"]' ).addClass( 'zakra-hide' );
							} else {
								$( '.control-section[aria-owns="' + parentId + '"]' ).removeClass( 'zakra-hide' );
							}
						}
					}
				);
		...
		$(
			function () {
				Zakra_Customizer_FrameWork.init();
			}
		);
