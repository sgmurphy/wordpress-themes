...
(
	function ( $ ) {
		...
		wp.customize.controlConstructor['zakra-date'] = wp.customize.Control.extend( {
...
(
	function ( $ ) {
		...
		wp.customize.controlConstructor['zakra-gradient'] = wp.customize.Control.extend(
			{
				...
				initColorControl : function () {
					...
					var control     = this,
						value       = control.setting._value,
						colorpicker = control.container.find( '.color .zakra-color-picker-alpha' );
				...
				initColor2Control : function () {
					...
					var control     = this,
						value       = control.setting._value,
						colorpicker = control.container.find( '.color-2 .zakra-color-picker-alpha' );
		...
		wp.customize.controlConstructor[ 'zakra-group' ] = wp.customize.Control.extend(
			{
					...
					control.container.on(
						...
						function ( e ) {
							...
							if ( $this.hasClass( 'open' ) ) {
								...
								let get_open_popup = parent_section.find( '.zakra-group-toggle-icon.open' );
								...
								if ( is_loaded ) {
									...
									let fields     = control.params.zakra_fields,
				...
				zakra_render_field: function ( wrap, fields, control_element ) {
					...
					let control = this;
					let zakra_field_wrap = wrap.find( '.zakra-fields-wrap' );
					let fields_html = '';
					let control_types = [];
				...
				initColorControl: function ( wrap, control_elem, name ) {
					...
					let control = this;
					let colorpicker = wrap.find( '.customize-control-zakra-color .zakra-color-picker-alpha' );
					...
					colorpicker.wpColorPicker(
						{
							change: function ( event, ui ) {
								...
								if ( 'undefined' != typeof event.originalEvent || 'undefined' != typeof ui.color._alpha ) {
									...
									let picker = $( '#' + name + '.customize-control-zakra-color .zakra-color-picker-alpha' );
				...
				initTypographyControl: function ( control, control_atts, name ) {
					...
					controlContainer.on(
						...
						function () {
							let inputValue     = jQuery( this ),
								...
								selector       = wrapper.find( '.zakra-font-size-' + device + '-warning' ),
					...
					controlContainer.on(
						...
						function () {
							let inputValue     = jQuery( this ),
								...
								input          = wrapper.find( 'input[type=number]' ),
								device         = input.data( 'device' ),
								selector       = wrapper.find( '.zakra-line-height-' + device + '-warning' ),
								setRangeValue  = range.val( value ),
								sliderValue    = `linear-gradient(to right, #0073AA 0%, #0073AA ${convertedValue}%, #ebebeb ${convertedValue}%, #ebebeb 100%)`;
					...
					controlContainer.on(
						...
						function () {
							let inputValue     = jQuery( this ),
								...
								input          = wrapper.find( 'input[type=number]' ),
								device         = input.data( 'device' ),
								selector       = wrapper.find( '.zakra-letter-spacing-' + device + '-warning' ),
								setRangeValue  = range.val( value ),
								sliderValue    = `linear-gradient(to right, #0073AA 0%, #0073AA ${convertedValue}%, #ebebeb ${convertedValue}%, #ebebeb 100%)`;
					...
					this.container.find( '.font-size input[type=range]' ).on(
						...
						function () {
							...
							let slider         = jQuery( this ),
								...
								minVal         = slider.attr( 'min' ),
								convertedValue = ( value - minVal ) / ( maxVal - minVal ) * 100,
								input_number   = jQuery( this ).closest( '.slider-wrapper' ).find( '.zakra-range-value .input-wrapper input' ),
								sliderValue    = `linear - gradient( to right, #0073AA 0 % , #0073AA ${convertedValue} % , #ebebeb ${convertedValue} % , #ebebeb 100 % )`;
					...
					control.container.on('click', '.zakra-font-size-reset', function () {
						resetControlValues('font-size');
					});

					control.container.on('click', '.zakra-line-height-reset', function () {
						resetControlValues('line-height');
					});

					control.container.on('click', '.zakra-letter-spacing-reset', function () {
...
wp.customize.controlConstructor[ 'zakra-slider' ] = wp.customize.Control.extend( {
	...
	ready: function () {
		...
		let control        = this,
			slider         = this.container.find( '.zakra-progress' ),
		...
		control.container.on( 'change keyup paste input', '.size input', function () {
			let inputValue     = jQuery( this ),
				...
				wrapper     = jQuery( this ).closest( '.slider-wrapper' ),
				range = wrapper.find( 'input[type=range]' ),
				selector = wrapper.find( '.zakra-warning' ),
				setRangeValue  = range.val( value ),
				sliderValue    = `linear-gradient(to right, #0073AA 0%, #0073AA ${convertedValue}%, #ebebeb ${convertedValue}%, #ebebeb 100%)`;
		...
		this.container.find( '.zakra-slider-reset' ).click( function () {
			...
			let wrapper       = jQuery( this ).closest( 'li' ).children( '.slider-label' ),
				...
				input_number  = wrapper.find( '.zakra-range-value input' ),
