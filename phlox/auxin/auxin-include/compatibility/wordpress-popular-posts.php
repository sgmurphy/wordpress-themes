	...
	foreach( $mostpopular as $popular ) {
		...
		if ( $instance['stats_tag']['comment_count'] ) {
			...
			$stats[] = '<span class="wpp-comments">' . sprintf(
				_n( '1 comment', '%s comments', $popular->comment_count, 'phlox' ),
		...
		if ( $instance['stats_tag']['views'] ) {
			...
			if ($instance['order_by'] == 'avg') {
				...
				$stats[] = '<span class="wpp-views">' . sprintf(
					_n( '1 view per day', '%s views per day', intval($popular->pageviews), 'phlox' ),
			...
			// Sorting posts by views
				...
				// display text in singular or plural, according to views count
				$stats[] = '<span class="wpp-views">' . sprintf(
					_n( '1 view', '%s views', intval($popular->pageviews), 'phlox' ),
		...
		if ( $instance['stats_tag']['author'] ) {
			...
			$stats[]      = '<span class="wpp-author">' . sprintf( __( 'by %s', 'phlox' ), $display_name ). '</span>';
		...
		if ( $instance['stats_tag']['date']['active'] ) {
			...
			$stats[] = '<span class="wpp-date">' . sprintf( __( 'posted on %s', 'phlox' ), $date ) . '</span>';
		...
		if ( $instance['stats_tag']['category'] ) {
			...
			if ( $post_cat != '' ) {
				$stats[] = '<span class="wpp-category">' . sprintf( __( 'under %s', 'phlox' ), $post_cat ) . '</span>';
			}
